              ...
              <div class="coupitem" v-for="(item,index) in couptlist"  :key="index">
                ...
                <div class="coupinfo">
                    ...
                    <div class="right flex-container">
                        ...
                        <div class="btn" v-if="item.Enables===1&&item.isUse" @click="gotoShopcenter(item.Id,item.Denomination)">立即使用</div>
                    </div>
                </div>
              </div>
          </div>
    ...
    async useCoupon(){
      ...
      if(coupon.status){
        ...
        for(let i=0;i<list.length;i+=1){
          ...
          // 满足条件,isUse为可使用的优惠券
          if(
            ((_list.MeetConditions*1) <= (coupon.price*1)||(_list.MeetConditions*1)===0)&&
          ((_list.ProductId*1) === (coupon.productId*1)||(_list.ProductId*1)===0)&&
          ((_list.ProTypeId*1) === (coupon.classifyId*1)||(_list.ProTypeId*1)===0)
          )
          {
